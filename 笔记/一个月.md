...
ThreadPoolExecutor 线程池用于管理线程多个线程，减少线程创建销毁消耗，提高代码执行效率
...
2.public class PrintABCUsingLockCondition {
...
相当于轻量级的 synchronized，volatitle 能保证有序性（禁用指令重排序）、可见性。只能用来修饰变量，适用修饰可能被多线程同时访问的变量
...
> 为什么 HashMap 的容量是 2 的幂次方（参考问题 4）
...
@JvmOverloads主要是Kotlin定义默认参数生成重载方法；@file:JvmName指定Kotlin文件编译之后生成的类名。
...
方法调用：不空才执行后续代码，否则返回null；如果是!!，空的话直接抛出NPE异常。
...
>>>>>>> 4f23eef92b2aba5c8bde43c0f09f17d77f388420
