...
https://juejin.cn/post/6844904146684870669#comment RecyclerView 的缓存 AMS handler通信  CacheInterceptor源码
...
- AttachedScrap(屏幕内)：用于屏幕内ViewHolder快速重用
  ...
  - mChangedScrap：mChangedScrap和mAttachedScrap是同一级的缓存。mChangedScrap的调用场景是notifyItemChanged和notifyItemRangeChanged，只有发生变化的ViewHolder才会放入到mChangedScrap中。mChangedScrap缓存中的ViewHolder是需要调用onBindViewHolder方法重新绑定数据的。
