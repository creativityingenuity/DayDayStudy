...
Activity并不负责视图控制，它只是控制生命周期和处理事件。真正控制视图的是Window，window是视图的承载
一个Activity包含了一个Window，其具体的实现是 PhoneWindow， 通过WindowManager将DecorView加载进来，DecorView来加载Activity中设置的布局
在WindowManager添加DecorView的时候会创建ViewRootimpl,它是连接WindowManagerService和DecorView的纽带，所有View的绘制以及事件分发等交互都是通过它来执行或传递的。
