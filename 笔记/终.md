...
2. 在Application的attachBaseContext中将SplashActivity和MainActivity预加载
...
对application的oncreate里面初始化的第三方依赖异步初始化，有两个问题：

1. 如果两个异步之间执行的内容有着依赖关系
2. 如果其中一个异步处理，需要先执行完毕自己，主线程才能继续往下执行，那么常规方式也不好处理
