...
- 2、Looper是一个死循环，会不断的从MessageQueue提取触发条件的Message，并将Message交给对应的target handler来处理。如果Messagequeue没有消息或者还没有到执行第一条消息的时间，会执行idleHanlder任务，若是idleHandler没有任务就调用Native层的epoll机制进行休眠
